require "administrate/base_dashboard"

class MessagesConfigurationDashboard < Administrate::BaseDashboard
  # ATTRIBUTE_TYPES
  # a hash that describes the type of each of the model's fields.
  #
  # Each different type represents an Administrate::Field object,
  # which determines how the attribute is displayed
  # on pages throughout the dashboard.
  ATTRIBUTE_TYPES = {
    id: Field::Number,
    direction_names: Field::String,
    display_department_logo: Field::Boolean,
    display_europe_logos: Field::Boolean,
    display_pole_emploi_logo: Field::Boolean,
    help_address: Field::String,
    letter_sender_name: Field::String,
    organisation: Field::BelongsTo,
    sender_city: Field::String,
    signature_lines: Field::String,
    sms_sender_name: Field::String,
    created_at: Field::DateTime,
    updated_at: Field::DateTime
  }.freeze

  # COLLECTION_ATTRIBUTES
  # an array of attributes that will be displayed on the model's index page.
  #
  # By default, it's limited to four items to reduce clutter on index pages.
  # Feel free to add, remove, or rearrange items.
  COLLECTION_ATTRIBUTES = %i[
    id
    direction_names
    display_department_logo
    display_europe_logos
  ].freeze

  # SHOW_PAGE_ATTRIBUTES
  # an array of attributes that will be displayed on the model's show page.
  SHOW_PAGE_ATTRIBUTES = %i[
    id
    direction_names
    display_department_logo
    display_europe_logos
    display_pole_emploi_logo
    help_address
    letter_sender_name
    organisation
    sender_city
    signature_lines
    sms_sender_name
    created_at
    updated_at
  ].freeze

  # FORM_ATTRIBUTES
  # an array of attributes that will be displayed
  # on the model's form (`new` and `edit`) pages.
  FORM_ATTRIBUTES = %i[
    direction_names
    display_department_logo
    display_europe_logos
    display_pole_emploi_logo
    help_address
    letter_sender_name
    organisation
    sender_city
    signature_lines
    sms_sender_name
  ].freeze

  # COLLECTION_FILTERS
  # a hash that defines filters that can be used while searching via the search
  # field of the dashboard.
  #
  # For example to add an option to search for open resources by typing "open:"
  # in the search field:
  #
  #   COLLECTION_FILTERS = {
  #     open: ->(resources) { resources.where(open: true) }
  #   }.freeze
  COLLECTION_FILTERS = {}.freeze

  # Overwrite this method to customize how messages configurations are displayed
  # across all pages of the admin dashboard.
  #
  # def display_resource(messages_configuration)
  #   "MessagesConfiguration ##{messages_configuration.id}"
  # end
end
