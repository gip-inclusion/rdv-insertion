class NotifyParticipationsToUsersJob < ApplicationJob
  def perform(participation_ids, event_to_notify)
    ...
    participations.each do |participation|
      NotifyParticipationToUserJob.perform_later(participation.id, "sms", notification_event) \
        if participation.phone_number_is_mobile?
      NotifyParticipationToUserJob.perform_later(participation.id, "email", notification_event) \
