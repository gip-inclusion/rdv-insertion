...
export default class extends Controller {
  ...
  async submit(event) {
    ...
    if (file) {
      if(!validateFileFormat(file, this.inputTarget.accept.split(',').map((format) => format.trim()))) {
        return
      }
  ...
  #retrieveRowsCnafData() {
    this.userList.forEach((user) => {
      ...
      const cnafDataRow = this.cnafDataRows.find((cnafDataRow) => {
      ...
      if (cnafDataRow) {
        const parsedCnafData = parseContactsData(cnafDataRow)
