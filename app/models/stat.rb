class Stat
  ...
  def percentage_of_applicants_oriented_in_time
    (applicants_oriented_in_less_than_30_days.length / (applicants_orientable_in_time.length.nonzero? || 1).to_f) * 100
  end

  def applicants_oriented_in_less_than_30_days
    relevant_applicants.to_a.select do |applicant|
  ...
  def average_orientation_delay_in_days
    ...
    relevant_applicants.rdv_seen.to_a.each do |applicant|
  ...
  def average_rdv_delay_in_days
    ...
    seen_rdvs = relevant_rdvs.seen.to_a

    seen_rdvs.to_a.each do |rdv|
      cumulated_rdv_delays += rdv.delay_in_days
    end

    cumulated_rdv_delays / (seen_rdvs.length.nonzero? || 1).to_f
  end
