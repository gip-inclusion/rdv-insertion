class BlockedUser < ApplicationRecord
  ...
  scope :grouped_by_day, lambda { |starts_at = Time.zone.parse("01/01/2025"), ends_at = Time.zone.now|
    where(created_at: starts_at..ends_at).group(Arel.sql("DATE_TRUNC('day', created_at)"))
