...
module Stats
  class ComputeStats < BaseService
    def initialize(applicants:, agents:, invitations:, rdvs:, rdv_contexts:, organisations:)
    ...
    def stats_data
      {
        ...
        average_time_between_rdv_creation_and_start_in_days_grouped_by_month:
          average_time_between_rdv_creation_and_start_in_days_grouped_by_month,
        percentage_of_applicants_oriented_in_less_than_30_days: percentage_of_applicants_oriented_in_less_than_30_days,
    ...
    def applicants_count
      @applicants.to_a.count
    end

    def applicants_count_grouped_by_month
      @applicants.group_by_month(:created_at, format: "%m/%Y", current: false).count
    end
    ...
    def rdvs_count_grouped_by_month
      @rdvs.group_by_month(:created_at, format: "%m/%Y", current: false).count
    end

    def sent_invitations_count
      sent_invitations.to_a.count
    end
