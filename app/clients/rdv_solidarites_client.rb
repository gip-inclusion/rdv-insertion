class RdvSolidaritesClient
  ...
  def create_user(request_body)
    post("/api/v1/users", request_body.to_json)
  end

  def get_user(user_id)
    get("/api/rdvinsertion/users/#{user_id}")
  end

  def create_user_profiles(user_id, organisation_ids)
    post("/api/rdvinsertion/user_profiles/create_many", {
      user_id: user_id, organisation_ids: organisation_ids
    }.to_json)
  end

  def create_referent_assignations(user_id, agent_ids)
    post("/api/rdvinsertion/referent_assignations/create_many", {
      user_id: user_id, agent_ids: agent_ids
    }.to_json)
  end

  def get_user_referent_assignations(user_id)
    get("/api/rdvinsertion/users/#{user_id}/referent_assignations")
  end

  def update_user(user_id, request_body = {})
    patch("/api/v1/users/#{user_id}", request_body.to_json)
  end
  ...
  def update_organisation(organisation_id, request_body = {})
    patch("/api/v1/organisations/#{organisation_id}", request_body.to_json)
  end

  def delete_user_profile(user_id, organisation_id)
    delete("/api/v1/user_profiles", { user_id: user_id, organisation_id: organisation_id })
  end

  def get_organisations(geo_params = {})
    get("/api/v1/organisations", geo_params)
  end

  def invite_user(user_id, request_body = {})
    post("/api/v1/users/#{user_id}/rdv_invitation_token", request_body.to_json)
  end

  def get_creneau_availability(link_params = {})
    get("/api/rdvinsertion/invitations/creneau_availability", link_params)
  end
  ...
  def create_referent_assignation(user_id, agent_id)
    post("/api/v1/referent_assignations", {
      user_id: user_id, agent_id: agent_id
    }.to_json)
  end

  def delete_referent_assignation(user_id, agent_id)
    delete("/api/v1/referent_assignations", { user_id: user_id, agent_id: agent_id })
  end
  ...
  def create_webhook_endpoint(organisation_id, subscriptions, trigger)
    post("/api/v1/organisations/#{organisation_id}/webhook_endpoints", {
      target_url: "#{ENV['HOST']}/rdv_solidarites_webhooks",
  ...
  def update_webhook_endpoint(webhook_endpoint_id, organisation_id, subscriptions, trigger)
    patch("/api/v1/organisations/#{organisation_id}/webhook_endpoints/#{webhook_endpoint_id}", {
      target_url: "#{ENV['HOST']}/rdv_solidarites_webhooks",
      secret: ENV["RDV_SOLIDARITES_SECRET"],
      subscriptions: subscriptions,
      trigger: trigger
  ...
  def update_participation(participation_id, request_body = {})
    patch("/api/v1/participations/#{participation_id}", request_body.to_json)
  end

  def create_motif_category(request_body)
    post("/api/rdvinsertion/motif_categories", request_body.to_json)
  end

  def create_motif_category_territory(motif_category_short_name, organisation_id)
    post("/api/rdvinsertion/motif_category_territories", {
  ...
  def delete(path, params = {}) = request(:delete, path, params)

  def request(http_method, path, payload)
    response = execute(http_method, path, payload)
    return response unless response.status == 401 && @authentication.renewable?

    @authentication.renew!
    execute(http_method, path, payload)
  end

  def execute(http_method, path, payload)
    connection.public_send(http_method, path, payload) do |req|
      req.headers.merge!(@authentication.headers)
    end
  end
