describe Users::ActivityChecker, type: :service do
  ...
  describe "#user_has_recent_activity?" do
    let!(:inactive_user) { create(:user, created_at: old_date) }
    let!(:active_user) { create(:user, created_at: recent_date) }
    ...
    context "when user has no recent activity" do
      it "returns false" do
        expect(service.user_has_recent_activity?(inactive_user)).to be false
      end
    end

    context "when user has recent activity" do
      it "returns true" do
        expect(service.user_has_recent_activity?(active_user)).to be true
      end
    end
    ...
    context "when user has recent invitation" do
      ...
      it "returns true" do
        expect(service.user_has_recent_activity?(inactive_user)).to be true
      end
    end
