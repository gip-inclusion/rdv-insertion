...
describe "Users API", swagger_doc: "v1/api.json" do
  ...
  path "api/v1/organisations/{rdv_solidarites_organisation_id}/users/create_many" do
    post "create users" do
      tags "User"
      consumes "application/json"
      produces "application/json"
      description "Créé une liste d'usagers sans les inviter.
      La création se fait de manière asynchrone."
      ...
      before { allow(CreateUserJob).to receive(:perform_later) }
      ...
      response 200, "succès" do
        ...
        run_test! do
          expect(CreateUserJob).to have_received(:perform_later)
            .with(
              organisation.id,
              user1_params.merge(creation_source_attributes)
            )
  ...
  path "api/v1/organisations/{rdv_solidarites_organisation_id}/users/invite_many" do
    post "invite multiple users" do
      tags "User"
      consumes "application/json"
      produces "application/json"
      description "Invite une liste d'usagers existants à prendre rdv.
      L'invitation se fait de manière asynchrone. Chaque usager peut avoir ses propres paramètres d'invitation."

      parameter name: :rdv_solidarites_organisation_id, in: :path, type: :string,
                description: "L'id rdv-solidarites d'une organisation",
                example: "403023", required: true

      parameter name: :invitation_params, in: :body, required: true, schema: {
        type: "object",
        properties: {
          users: {
            type: "array",
            items: {
              type: "object",
              properties: {
                id: { type: "integer", description: "ID de l'usager à inviter" },
                invitation: {
                  type: "object",
                  properties: {
      ...
      let!(:invitation_params) do
        {
          users: [
            ...
            },
            {
              id: user2.id,
              invitation: {
                rdv_solidarites_lieu_id: 456,
                motif_category: { name: "Orientation pro" }
              }
            }
          ]
        }
      ...
      before { allow(InviteUserJob).to receive(:perform_later) }
        ...
        run_test! do
          expect(InviteUserJob).to have_received(:perform_later)
            .with(
              user1.id,
              organisation.id,
              hash_including(rdv_solidarites_lieu_id: 123),
              hash_including(name: "RSA orientation")
            )
          expect(InviteUserJob).to have_received(:perform_later)
            .with(
              user2.id,
