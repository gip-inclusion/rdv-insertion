describe OrganisationsController do
  ...
  describe "#update_infos" do
    context "when valid params" do
      ...
      it "updates the organisation and redirects to show_infos" do
        patch :update_infos, params: {
          id: organisation.id,
          organisation: { name: "Nouveau nom", phone_number: "0607080910" }
        }

        expect(response).to redirect_to(show_infos_organisation_path(organisation))
      end
    end
    ...
    context "when invalid params" do
      before do
        allow(Organisations::Update).to receive(:call)
          .and_return(OpenStruct.new(success?: false, errors: ["Erreur"]))
      end

      it "renders edit_infos with unprocessable_entity status" do
        patch :update_infos, params: {
          id: organisation.id,
          organisation: { name: "" }
        }
  ...
  describe "#update_data_retention" do
    ...
    context "when valid duration" do
      it "updates the data retention duration and redirects to show_data_retention" do
        patch :update_data_retention, params: {
          id: organisation.id,
          organisation: { data_retention_duration_in_months: 12 }
        }

        expect(response).to redirect_to(show_data_retention_organisation_path(organisation))
        expect(organisation.reload.data_retention_duration_in_months).to eq(12)
      end
    end

    context "when invalid duration" do
      it "renders edit_data_retention with unprocessable_entity status" do
        patch :update_data_retention, params: {
          id: organisation.id,
