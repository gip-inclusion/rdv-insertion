Stat.find_each do |stat|
  stat.rate_of_no_show_for_invitations =
    Stats::ComputeRateOfNoShow.call(participations: stat.participations_after_invitations_set).value
  stat.rate_of_no_show_for_convocations =
    Stats::ComputeRateOfNoShow.call(participations: stat.participations_with_notifications_set).value
  ...
  while date < Time.zone.parse("31/08/2023 12:00")
    ...
    rate_for_invitations_for_date =
      Stats::ComputeRateOfNoShow.call(
        participations: stat.participations_after_invitations_set.where(created_at: date.all_month)
      ).value.round
    ...
    rate_for_convocations_for_date =
      Stats::ComputeRateOfNoShow.call(
        participations: stat.participations_with_notifications_set.where(created_at: date.all_month)
      ).value.round
